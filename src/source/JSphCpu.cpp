  ...
  ,TpShifting shiftmode,tfloat4 *shiftposfs)const
{
  ...
  const int pfin=int(pinit+n);
  float* visc=(float*)malloc(Np*sizeof(float));
  float* arp1=(float*)malloc(Np*sizeof(float));
  float* deltap1=(float*)malloc(Np*sizeof(float));
  tfloat3* acep1=(tfloat3*)malloc(Np*sizeof(tfloat3));
  tfloat4* shiftposfsp1=(tfloat4*)malloc(Np*sizeof(tfloat4));
  tsymatrix3f* gradvelp1=(tsymatrix3f*)malloc(Np*sizeof(tsymatrix3f));
  bool* ftp1=(bool*)malloc(Np*sizeof(bool));

  #pragma acc data create(visc[0:Np], arp1[0:Np], deltap1[0:Np], acep1[0:Np], shiftposfsp1[0:Np], gradvelp1[0:Np], ftp1[0:Np]) \
                   copy(viscdt, shiftposfs[0:Np], delta[0:Np], ar[0:Np], ace[0:Np], gradvel[0:Np]) \
                   copyin(dcell[0:Np], divdata, divdata.begincell[0:CellDiv->GetSizeBeginCell()], code[0:Np], pos[0:Np], verlhop[0:Np], FtObjs[0:FtCount], press[0:Np], velrhop[0:Np], tau[0:Np])
  {
    // 変数の初期化など
    #pragma acc parallel loop
    for(int p1=int(pinit);p1<pfin;p1++){
      visc[p1]=0,arp1[p1]=0,deltap1[p1]=0;
      acep1[p1]=TFloat3(0);
      gradvelp1[p1]={0,0,0,0,0,0};

      //-Variables for Shifting.
      if(shift)shiftposfsp1[p1]=shiftposfs[p1];

      //-Obtain data of particle p1 in case of floating objects. | Obtiene datos de particula p1 en caso de existir floatings.
      ftp1[p1]=false;     //-Indicate if it is floating. | Indica si es floating.
      if(USE_FLOATING){
        ftp1[p1]=CODE_IsFloating(code[p1]);
        if(ftp1[p1] && tdensity!=DDT_None)deltap1[p1]=FLT_MAX; //-DDT is not applied to floating particles.
        if(ftp1[p1] && shift)shiftposfsp1[p1].x=FLT_MAX;  //-For floating objects do not calculate shifting. | Para floatings no se calcula shifting.
      }
    }
    ...
    for(int p1=int(pinit);p1<pfin;p1++){
      ...
      const tsymatrix3f taup1=(tvisco==VISCO_Artificial? gradvelp1[p1]: tau[p1]);
      const bool rsymp1=(Symmetry && posp1.y<=KernelSize); //<vs_syymmetry>
      
      //隣接セル探索ループの直前で粒子リストを作成
      tdouble3 particleArray_pos[3993];// 座標 (サイズ Np)
      tfloat4 particleArray_velrhop[1331];// 速度となんかの複合とかだったような気がする (サイズ Np)
      typecode particleArray_code[1331];// 特殊識別用マーカー (サイズ Np)
      float particleArray_press[1331];// 圧力 (サイズ Np)
      tsymatrix3f particleArray_tau[7986]; // 粘度計算用 (サイズ Np)
      ...
      for(int z=ngs.zini;z<ngs.zfin;z++)for(int y=ngs.yini;y<ngs.yfin;y++){
        ...
        //粒子リストを作成
        for(unsigned p2 = pif.x; p2 < pif.y; p2++) {
          particleArray_pos[particleCount] = pos[p2];
          particleArray_code[particleCount] = code[p2];
          particleArray_velrhop[particleCount] = velrhop[p2];
          particleArray_press[particleCount] = press[p2];
          particleArray_tau[particleCount] = tau[p2]; 
            //printf("pos[p2]: x=%.2f,particleArray[particleCount]: x=%.2f\n:" ,pos[p2].x,particleArray[particleCount].x);//確認用
            //int a; std::cin >> a; // 入力待ちで実行を止める
          particleCount++;
          ...
          if(rr2<=KernelSize2 && rr2>=ALMOSTZERO){
            ...
            if(USE_FLOATING){
              ...
              if(ftp2)massp2=FtObjs[CODE_GetTypeValue(code[p2])].massp;
                ...
                if(ftp2 && tdensity==DDT_DDT && massp2<=(MassFluid*1.2f))deltap1[p1]=FLT_MAX;
              #else
                if(ftp2 && tdensity==DDT_DDT)deltap1[p1]=FLT_MAX;
              #endif
              if(ftp2 && shift && shiftmode==SHIFT_NoBound)shiftposfsp1[p1].x=FLT_MAX; //-With floating objects do not use shifting. | Con floatings anula shifting.
            ...
            if(compute){
              ...
              acep1[p1].x+=p_vpm*frx; acep1[p1].y+=p_vpm*fry; acep1[p1].z+=p_vpm*frz;
            }
            //-Density derivative (Continuity equation).
            const float dvx=velp1.x-velrhop2.x, dvy=velp1.y-velrhop2.y, dvz=velp1.z-velrhop2.z;
            if(compute)arp1[p1]+=massp2*(dvx*frx+dvy*fry+dvz*frz)*(rhopp1/velrhop2.w);

            const float cbar=(float)Cs0;
            //-Density Diffusion Term (Molteni and Colagrossi 2009).
            if(tdensity==DDT_DDT && deltap1[p1]!=FLT_MAX){
              ...
              deltap1[p1]=(boundp2 && TBoundary==BC_DBC? FLT_MAX: deltap1[p1]+delta);
            }
            //-Density Diffusion Term (Fourtakas et al 2019).
            if((tdensity==DDT_DDT2 || (tdensity==DDT_DDT2Full && !boundp2)) && deltap1[p1]!=FLT_MAX && !ftp2){
              ...
              deltap1[p1]=(boundp2? FLT_MAX: deltap1[p1]-delta); //-blocks it makes it boil - bloody DBC
            }

            //-Shifting correction.
            if(shift && shiftposfsp1[p1].x!=FLT_MAX){
              ...
              shiftposfsp1[p1].x=(noshift? FLT_MAX: shiftposfsp1[p1].x+massrhop*frx); //-For boundary do not use shifting. | Con boundary anula shifting.
              shiftposfsp1[p1].y+=massrhop*fry;
              shiftposfsp1[p1].z+=massrhop*frz;
              shiftposfsp1[p1].w-=massrhop*(drx*frx+dry*fry+drz*frz);
            }
            ...
            if(compute){
              ...
              if(tvisco==VISCO_Artificial){//-Artificial viscosity.
                if(dot<0){
                  ...
                  acep1[p1].x-=pi_visc*frx; acep1[p1].y-=pi_visc*fry; acep1[p1].z-=pi_visc*frz;
                }
              }
                ...
                {//-Laminar contribution.
                  ...
                  acep1[p1].x+=vtemp*dvx; acep1[p1].y+=vtemp*dvy; acep1[p1].z+=vtemp*dvz;
                }
                ...
                acep1[p1].x+=massp2*(tau_xx*frx + tau_xy*fry + tau_xz*frz);
                acep1[p1].y+=massp2*(tau_xy*frx + tau_yy*fry + tau_yz*frz);
                acep1[p1].z+=massp2*(tau_xz*frx + tau_yz*fry + tau_zz*frz);
                //-Velocity gradients.
                if(!ftp1){//-When p1 is a fluid particle.
                  const float volp2=-massp2/velrhop2.w;
                  float dv=dvx*volp2; gradvelp1[p1].xx+=dv*frx; gradvelp1[p1].xy+=dv*fry; gradvelp1[p1].xz+=dv*frz;
                        dv=dvy*volp2; gradvelp1[p1].xy+=dv*frx; gradvelp1[p1].yy+=dv*fry; gradvelp1[p1].yz+=dv*frz;
                        dv=dvz*volp2; gradvelp1[p1].xz+=dv*frx; gradvelp1[p1].yz+=dv*fry; gradvelp1[p1].zz+=dv*frz;
    ...
    #pragma acc parallel loop reduction(max:viscdt)
    for(int p1=int(pinit);p1<pfin;p1++){
      //-Sum results together. | Almacena resultados.
      if(shift||arp1[p1]||acep1[p1].x||acep1[p1].y||acep1[p1].z||visc[p1]){
        if(tdensity!=DDT_None){
          if(delta)delta[p1]=(delta[p1]==FLT_MAX || deltap1[p1]==FLT_MAX? FLT_MAX: delta[p1]+deltap1[p1]);
          else if(deltap1[p1]!=FLT_MAX)arp1[p1]+=deltap1[p1];
        }
        ar[p1]+=arp1[p1];
        ace[p1]=ace[p1]+acep1[p1];
        viscdt=max(viscdt,visc[p1]);
        if(tvisco==VISCO_LaminarSPS){
          gradvel[p1].xx+=gradvelp1[p1].xx;
          gradvel[p1].xy+=gradvelp1[p1].xy;
          gradvel[p1].xz+=gradvelp1[p1].xz;
          gradvel[p1].yy+=gradvelp1[p1].yy;
          gradvel[p1].yz+=gradvelp1[p1].yz;
          gradvel[p1].zz+=gradvelp1[p1].zz;
        }
        if(shift)shiftposfs[p1]=shiftposfsp1[p1];
      }
    }
  }
  ...
  // タイマーの停止
    ...
    Log->Print("InteractionForcesFluid: " + std::to_string(elapsed.count()) + " seconds");
}
  ...
  for(int cf=0;cf<nft;cf++){
    ...
    if(p1!=UINT_MAX){
      ...
      if(acep1.x||acep1.y||acep1.z){
        ace[p1]=ace[p1]+acep1;
        //onst int th=omp_get_thread_num();if(demdtth[th*OMP_STRIDE]<demdtp1)demdtth[th*OMP_STRIDE]=demdtp1;
      viscdt=max(viscdt,demdtp1);
      }
    }
  }
